    ...
    try {
        ...
        if(req.session.user){
           ...
           const flash= req.flash('flash')
            res.render('profile',{listedCategory,products,login:req.session.user,userdata,msg:flash})
        }else{
...
const editProfile = async ( req , res ) => {
    try {
       ...
       const updateData = await User.findByIdAndUpdate({_id:id},{$set:{fullname:name,phone:phones}})
       if(updateData){
